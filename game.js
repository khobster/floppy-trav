...
function updateBirdPosition() {
  ...
  if (birdY < 0) birdY = 0;
  if (birdY + spriteSheet.height >= canvas.height) gameOver();
}

// Draw the bird
function drawBird() {
  const frameX = spriteFrames[currentFrameIndex].x;
  ctx.drawImage(spriteSheet, frameX, 0, spriteSheet.width / 3, spriteSheet.height, birdX, birdY, spriteSheet.width / 3, spriteSheet.height);
}

// Update the frame for sprite animation
function updateFrame() {
  frameCount++;
  if (frameCount >= 15) { // Update to control frame rate
    frameCount = 0;
...
function updatePipes() {
  ...
  pipes.forEach(function(pipe, index) {
    ...
    if (pipe.x + pipe.width < 0) {
      ...
      score++;  // Score increases when pipes are passed
    }
  });
}
...
function checkCollisions() {
  for (let i = 0; i < pipes.length; i++) {
    let pipe = pipes[i];
    let hitTopPipe = birdX < pipe.x + pipe.width && birdX + spriteSheet.width / 3 > pipe.x && birdY < pipe.top;
    let hitBottomPipe = birdX < pipe.x + pipe.width && birdX + spriteSheet.width / 3 > pipe.x && birdY + spriteSheet.height > canvas.height - pipe.bottom;
...
function gameLoop() {
  if (!gameRunning) return; // Do not run the loop until the game starts
  ...
  ctx.fillStyle = 'white';
  ctx.font = '20px Arial';
  ctx.fillText('Score: ' + score, 10, 30); // Changed to be always visible
